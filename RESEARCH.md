...
choice, not a reimplementation of prior art, and one of the real bugs found and fixed
during this project's own security review (a cross-domain leak on `/api/v1/insights/*`
that returned every domain's data regardless of role, closed by extracting the same
scoping helper `/api/v1/kpis` already used into `_scope_kpi_df()` and applying it
everywhere) is direct evidence that retrieval-time enforcement, not UI-level hiding, is
the thing that actually has to be correct.
