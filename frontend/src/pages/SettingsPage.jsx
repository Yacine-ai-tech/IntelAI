...
const getScenarios = (t) => [
  ...
  { id: 'declining_financial', label: t('scenDecliningLbl') || 'Declining Financial', description: t('scenDecliningDesc') || 'Financial distress scenario' },
  { id: 'high_churn_crisis', label: t('scenChurnLbl') || 'High Churn Crisis', description: t('scenChurnDesc') || 'Customer retention crisis' },
  { id: 'operational_meltdown', label: t('scenOpsLbl') || 'Operational Meltdown', description: t('scenOpsDesc') || 'Operations failure' },
  { id: 'talent_crisis', label: t('scenTalentLbl') || 'Talent Crisis', description: t('scenTalentDesc') || 'HR/talent issues' },
  { id: 'cybersecurity_breach', label: t('scenCyberLbl') || 'Cybersecurity Breach', description: t('scenCyberDesc') || 'Security incident' },
  { id: 'esg_compliance_failure', label: t('scenEsgLbl') || 'ESG Compliance Failure', description: t('scenEsgDesc') || 'ESG governance issues' },
]
          ...
          <Panel title={t('scenarioManagement') || 'Benchmarking Scenarios'} icon={Database} style={{ marginTop: 18 }}>
            <div className="form-group">
              ...
              <select className="form-input" value={currentScenario} onChange={e => handleScenarioChange(e.target.value)} disabled={seeding}>
                {getScenarios(t).map(s => (
                  <option key={s.id} value={s.id}>{s.label}</option>
                ))}
              </select>
            </div>
            <div style={{ marginTop: 12, padding: 12, background: 'var(--surface-2)', borderRadius: 8, border: '1px solid var(--border)' }}>
              <h4 style={{ margin: '0 0 8px 0', fontSize: '.9rem', color: 'var(--text)' }}>{getScenarios(t).find(s => s.id === currentScenario)?.description}</h4>
